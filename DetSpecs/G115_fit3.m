function fitG115=G115_fit3(lgc_svplt)
% G115_fit2
% Let's fit TES parameters(fOp,gPep_v,Tc,w_Tc,T_bath) to measured dIdV and
% IV quantities(loop gain, Tau0, P0,R0,beta)+ the measured current noise
% Si(0)
...
%       - Tc until we find the correct noise [coupled with other 2 equations]
%       - w_Tc until we find loopgain
%       - Tbath until we find P0 [couple with the other equations]

guessX=[2.7,67e-3,1.4e-4,66e-3]; %[guess fgPep_v,guess Tc, guess wTc,Tbath guess];

options=optimset('fsolve');
options=optimset(options,'MaxFunEvals',2000);

vary= @(x)vary_gPep_Tc_wTc_Tbath(x,fitfOp,meastau0,measLG,measPo,measSi);
...
figure(11)
title({'G115 Noise','fitting \Sigma, T_{c},w_{Tc}, T_{bath} to S_{i}(0) and dIdV'})
if lgc_pltmeas
    hold on
    for jj=1:4
...
if lgc_svplt
    saveas(11,'G115_fit3.png','png')
end
 
end

function [dY,Det] = vary_gPep_Tc_wTc_Tbath(X,fitfOp,meastau0,measLG,measPo,measSi)
    ...
    [sigPt,Det]=SimulatedNoise_1TES(Det,false);
    
    %let's multiply the noise by 1e12 to make it scale 1
    dY = [Det.TES.tau0-meastau0; Det.TES.LG-measLG ; Det.TES.Po-measPo; 1e12*(sqrt(Det.Noise.Si.tot(1))-measSi)];
end
